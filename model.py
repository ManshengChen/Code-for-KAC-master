...
class KAC(nn.Module):
    def __init__(self, x_dim, n_cluster):
        super(KAC, self).__init__()
        
        self.encoder = AE_encoder(x_in_dim=x_dim, x_hidden_dim=args.out_dim).to('cuda')
        self.decoder = AE_decoder(x_hidden_dim=args.out_dim, x_in_dim=x_dim).to('cuda')
        self.S_net = MLP_fusion(S_in_dims=2 * args.out_dim, S_hidden_dim=args.out_dim).to('cuda') 

        self.cluster_layer = nn.Parameter(torch.Tensor(n_cluster, args.out_dim), requires_grad=True)
    ...
    def forward(self, x, Z_g):
        ...
        return x_hat, Z_a, Z_g, S, q
    


class AE_encoder(torch.nn.Module): 
    def __init__(self, x_in_dim, x_hidden_dim):
        super(AE_encoder, self).__init__()
        self.fc1 = torch.nn.Linear(x_in_dim, x_hidden_dim)
        self.fc2 = torch.nn.Linear(x_hidden_dim, 2*x_hidden_dim)
        self.fc3 = torch.nn.Linear(2*x_hidden_dim, x_hidden_dim)
        self.act = nn.Sigmoid()
...
class AE_decoder(nn.Module):
    ...
    def __init__(self, x_hidden_dim, x_in_dim):
        super(AE_decoder, self).__init__()

        self.dec1 = torch.nn.Linear(x_hidden_dim, 2*x_hidden_dim,)
        self.dec2 = torch.nn.Linear(2*x_hidden_dim, x_hidden_dim,)
        self.dec3 = torch.nn.Linear(x_hidden_dim, x_in_dim)
        # self.act = nn.LeakyReLU(0.2, inplace=True)
        self.act = nn.Sigmoid()

    def forward(self, z_ae):
        z = self.act(self.dec1(z_ae))
        z = self.act(self.dec2(z))
        x_hat = self.dec3(z)
        return x_hat

class MLP_fusion(torch.nn.Module): 
    def __init__(self, S_in_dims, S_hidden_dim):
        super(MLP_fusion, self).__init__()
        self.fc1 = torch.nn.Linear(S_in_dims, S_hidden_dim)
        self.fc2 = torch.nn.Linear(S_hidden_dim, 2 * S_hidden_dim)
        self.fc3 = torch.nn.Linear(2 * S_hidden_dim, S_hidden_dim)
        # self.fc4 = torch.nn.Linear(S_hidden_dim,S_hidden_dim)
        # self.fc5 = torch.nn.Linear(S_hidden_dim,S_hidden_dim)
        self.act = nn.Sigmoid()
...
class MAGNN_nc_mb(nn.Module):
    ...
    def forward(self, g_list, features_list, type_mask, edge_metapath_indices_list, target_idx_list):
...
class MAGNN_nc(nn.Module):
    ...
    def forward(self, g_lists, features_list, type_mask, edge_metapath_indices_lists, target_node_indices):
